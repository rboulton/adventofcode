...
class Module:
    ...
    def cached_signal(self, pulse):
        global hit, miss
        pre_state = self.downstream()
        # print("pre_state", pre_state)
        pulse_repr = repr(pulse)
        post_state = self.cache.get((pre_state, pulse_repr))
        if post_state is not None:
            # print("hit -> ", repr(post_state))
            self.set_downstream(post_state)
            hit += 1
            return
        else:
            miss += 1
        pulses = tuple(self.signal(pulse))
        # print(self.name, pulses)
        for p in pulses:
            yield p
        post_state = self.downstream()
        # print(pre_state)
        # print(post_state)
        self.cache[(pre_state, pulse_repr)] = post_state
...
class Conjunction(Module):
    ...
    @state.setter
    def state(self, value):
        for k, ch in zip(list(sorted(self.inputs.keys())), value):
            self.inputs[k] = int(ch)
