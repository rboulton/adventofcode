...
def variations(springs, startpos, sequences, seqsofar, requiredseq):
    if len(sequences) == 0:
        '''
        strrep = ['.'] * len(springs)
        for seq, l in seqsofar:
            for i in range(seq, seq + l):
                strrep[i] = '#'
        strrep = ''.join(strrep)
        assert runs(strrep) == requiredseq, strrep
        print(strrep, requiredseq)
        '''
        return 1
    ...
    for i in range(startpos, len(springs) + 2 - sum(sequences) - len(sequences)):
        if possible(springs, i, firstseq):
            ...
            r = variations(springs, nextpos, tailseq, seqsofar + [(i, firstseq)], requiredseq)
